...
const BACKEND_API_CONFIG = {
  BASE_URL: (typeof localStorage !== "undefined" && localStorage.getItem("sentinel_backend_url")) || "http://192.168.220.97:8000",
...
const PRESET_SAMPLES = [
    {
        title: "Fake Bank Phishing",
        text: "URGENT: Unauthorized login detected on your bank account from Eastern Europe. Visit http://paypal-security.support.net/login?token=89102 immediately to verify your identity and enter your OTP to prevent complete account freeze within 2 hours."
    },
    {
        title: "Malicious .exe Invoice",
        text: "Dear Customer, your corporate software invoice #88902 is overdue. Download our secured invoice viewer at http://192.168.1.104/secure/invoice.pdf.exe and enter Single Sign-On credentials to authorize clearance before end of business today or legal notice will be filed."
    },
    {
        title: "Urgent OTP Theft",
        text: "Security Alert: Unauthorized fund transfer of $1,499 initiated. Reply immediately with your registered mobile OTP and password to cancel this transaction within 15 minutes."
    },
    {
        title: "Legitimate Order Query",
        text: "Hello support team, could you please provide an updated tracking estimate for shipment #ORD-77124? The status has been 'Departed Hub' for 3 days. Thank you for your assistance!"
    }
];
            ...
            try {
                ...
                try {
                    ...
                    if (!apiUrl.includes("localhost:5000") && !apiUrl.includes("127.0.0.1:5000")) {
                        try {
                            const localUrl = "http://localhost:5000/api/scan-and-save";
...
function runThreatPipeline(text) {
    ...
    rawUrls.forEach((raw) => {
        ...
        const isSuspicious = isIpAddress || lookalikeBrand || isShortener || isFreeHosting || isSuspiciousTld || hasApk || flags.length >= 2;
