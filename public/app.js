...
// ==========================================================================
const DEFAULT_BACKEND_URL = (typeof window !== "undefined" && window.location.origin && window.location.origin.startsWith("http")) 
  ? window.location.origin 
  : "http://localhost:3000";

// Ensure stale offline team IPs or port 5000 in localStorage are purged
let storedUrl = (typeof localStorage !== "undefined" && localStorage.getItem("sentinel_backend_url")) || "";
if (storedUrl && (storedUrl.includes("192.168.") || storedUrl.includes(":8000") || storedUrl.includes(":5000"))) {
  storedUrl = "";
  if (typeof localStorage !== "undefined") localStorage.removeItem("sentinel_backend_url");
}

const BACKEND_API_CONFIG = {
  BASE_URL: storedUrl || DEFAULT_BACKEND_URL,
...
// Sample Payloads for Company-Facing SOC & Customer Support
const SAMPLE_PAYLOADS = [
  {
    title: "Mandatory E2E Threat",
    vector: "text",
    text: "URGENT! Your account has been compromised. Click this link immediately and enter your username, password and OTP."
  },
  {
    title: "Phishing Domain",
    vector: "url",
    text: "http://novapay-security-verify.com/login"
  },
  {
    title: "Customer Refund / Churn",
    vector: "text",
    text: "I have called 4 times about my refund of $249! Your payment system failed and I want my money back immediately or I will report you to the consumer protection bureau!"
  },
  {
    title: "Invoice Payload .exe",
    vector: "email",
...
function analyzeThreat(text, vector) {
  ...
  extractedUrls.forEach(u => {
    const hasAuthPath = /(login|signin|verify|auth|security|account|update|confirm|token|sso|password|otp|banking|support|portal)/i.test(u.url);
    const hasSuspiciousKeywords = /(verify|security|update|login|support|secure|account|alert|banking)/i.test(u.hostname);
    ...
    if (hasAuthPath) {
      ...
      reasons.push({
        severity: "high",
        title: "Credential Harvesting / Auth Path Target",
        desc: `URL path targets sensitive authentication or verification endpoint '${u.url}'.`
      });
    }
    if (hasSuspiciousKeywords && !u.lookalikeBrand) {
      score += 42;
      reasons.push({
        severity: "high",
        title: "Deceptive Domain Formulation",
        desc: `Hostname '${u.hostname}' embeds deceptive security/verification lures designed to simulate official corporate portals.`
      });
    }
    ...
    // Force high severity if confirmed fake or deceptive link is present
    if (u.lookalikeBrand || u.hasExe || u.hasApk || u.isIp || (hasSuspiciousKeywords && hasAuthPath) || (u.isSuspiciousTld && hasAuthPath)) {
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        // If remote device is offline, check if local Sentinel backend is on port 3000
        if (!BACKEND_API_CONFIG.BASE_URL.includes("localhost:3000") && !BACKEND_API_CONFIG.BASE_URL.includes("127.0.0.1:3000")) {
          try {
            const localProbe = await fetch("http://localhost:3000/", { signal: AbortSignal.timeout(1200) });
            if (localProbe.ok) {
              BACKEND_API_CONFIG.BASE_URL = "http://localhost:3000";
              res = localProbe;
            }
          } catch (e3) {}
        }
...
function normalizeBackendResponse(raw, text, vector) {
  ...
  const candidateKeys = [
    ...
    raw?.riskAssessment?.riskScore,
    raw?.riskAssessment?.score,
    raw?.securityAnalysis?.threatScore,
    raw?.securityAnalysis?.score,
    raw?.record?.riskAssessment?.riskScore,
  ...
  const isExplicitThreat = raw?.is_phishing === true || raw?.is_threat === true || 
                          ...
                          raw?.securityAnalysis?.hasThreat === true;
  ...
  const isPhishTag = isExplicitThreat || 
                     ...
                     pred === "critical" || pred === "high" || raw?.riskAssessment?.riskLevel === "CRITICAL";
  ...
  if (isPhishTag) {
    backendScore = Math.max(backendScore !== null ? backendScore : 0, 92);
  ...
  return {
    ...
    customerIntelligence: (raw && raw.customerIntelligence) || null,
    category: (raw && (raw.category || (raw.customerIntelligence && raw.customerIntelligence.category))) || (isDanger ? "Urgent Security Threat" : "Customer Support Inquiry"),
    priority: (raw && (raw.priority || (raw.customerIntelligence && raw.customerIntelligence.priority))) || (isDanger ? "P1 Critical" : "P2 Normal"),
    sentiment: (raw && (raw.sentiment || (raw.customerIntelligence && raw.customerIntelligence.sentiment))) || (isDanger ? "Negative / Coercive" : "Neutral / Constructive"),
    sla: (raw && (raw.sla || (raw.customerIntelligence && raw.customerIntelligence.sla))) || (isDanger ? "Immediate SOC Containment" : "Standard 4h SLA"),
    custSummary: (raw && ((raw.customerIntelligence && raw.customerIntelligence.summary) || raw.summary)) || summary,
...
function renderResults(res) {
  ...
  // 1.4. Enterprise Customer Intelligence & SLA Card
  const custCategory = document.getElementById("res-cust-category");
  const custSentiment = document.getElementById("res-cust-sentiment");
  const custSla = document.getElementById("res-cust-sla");
  const custSummary = document.getElementById("res-cust-summary");
  const custPriorityTag = document.getElementById("res-cust-priority-tag");

  if (custCategory) custCategory.textContent = res.category || "General Support";
  if (custSentiment) {
    custSentiment.textContent = res.sentiment || "Neutral";
    custSentiment.style.color = (res.sentiment || "").toLowerCase().includes("neg") ? "var(--toxic-red)" : "var(--neon-green)";
  }
  if (custSla) custSla.textContent = res.sla || "Standard 4h Response";
  if (custSummary) custSummary.textContent = res.custSummary || res.summary || "Customer support analysis completed.";
  if (custPriorityTag) {
    const prio = (res.priority || "P2 NORMAL").toUpperCase();
    custPriorityTag.textContent = `Priority: ${prio}`;
    custPriorityTag.style.color = (prio.includes("P1") || prio.includes("CRIT")) ? "var(--toxic-red)" : "#a5b4fc";
  }
